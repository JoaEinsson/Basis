...
| D86 | Paper must define light-appearance lyric colors rather than inherit the dark registry defaults. The rendered active, past, upcoming, and translation lyric states must remain readable against the resolved Now Playing background after opacity is applied; the active line has the strongest emphasis. All lyric colors, active scale, and inactive opacity remain editable semantic Theme Engine tokens, and the editor must preview and warn about their effective contrast. |
| D87 | Lyric content never requires horizontal scrolling. Synchronized buttons, plain text, translations, and candidate copy wrap within the available lyric region, including long words through a safe overflow-wrap fallback. The wide layout may allocate additional lyric width while preserving a usable artwork region, but a horizontal lyric scrollbar is not an accepted fallback. |
| D88 | Now Playing provides an accessible Show/Hide Lyrics control. Hiding lyrics recomposes the canvas into a centered artwork-and-metadata presentation rather than leaving an empty column. The manual preference is device-local and survives track changes; an LRCLIB/local document explicitly marked instrumental automatically uses artwork-only presentation without overwriting that preference, so the previous manual state returns on the next non-instrumental track. Missing/error lyrics remain visible with their retry state and are not treated as instrumental. |
| D89 | A featured track credit must not split one release into duplicate album entities. Album identity and displayed album artist use the explicit album-artist tag when it contains a stable primary credit; otherwise they use the primary structured track artist and album-level consensus. Before changing normalization, capture the actual raw AlbumArtist/TrackArtists values from regression fixtures. As a narrow exception to D11, when a source exposes only one display string, a deterministic identity-only recognizer may remove a trailing `feat.`, `ft.`, or `featuring` clause if tracks with the same normalized album and compatible year corroborate the same primary credit. It never rewrites the stored/displayed track credit, creates artist relations from the suffix, splits on `&` or `/`, uses folders/network data, or combines ambiguous/conflicting releases. |
| D90 | The final Linux updater asset and detached signature use the stable names `Basis.AppImage` and `Basis.AppImage.sig`; the release tag, internal application version, and updater metadata remain versioned. Post-MVP Linux desktop integration is explicit and per-user: it defaults to `~/.local/bin/Basis.AppImage`, writes an XDG-compliant `basis.desktop` and icon without root privileges, and never silently modifies the application menu merely because an AppImage was launched. The workflow and Arch/KDE smoke must prove that a signed update preserves this stable path, executable permission, menu launch, icon, and relaunch behavior. |
